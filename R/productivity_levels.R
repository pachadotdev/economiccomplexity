...
#'
#' @export
#' @param trade_data matrix or tibble/data.frame (e.g. \code{world_trade_2017}).
...
#' @param gdp_data vector or tibble/data.frame
...
#' @param country2 string to indicate the column that contains exporting
#' countries in revealed_comparative_advantage (set to "country" by default)
#' @param value2 string to indicate the column that contains traded values in
#' revealed_comparative_advantage (set to "value" by default)
...
productivity_levels <- function(trade_data = NULL,
  ...
  if (!is.character(country1) & !is.character(product1) &
      !is.character(value1)) {
  ...
  if (nrow(trade_gdp) < nrow(unique(trade_data[, country1]))) {
    warning("Joining trade_data and gdp_data resulted in a table with less
    reporting countries than those in trade_data.")
  }

  if (nrow(trade_gdp) < nrow(gdp_data)) {
    warning("Joining trade_data and gdp_data resulted in a table with less
    reporting countries than those in gdp_data.")
  }

  # convert trade_gdp to matrix ----
  trade_gdp_rownames <- dplyr::select(trade_gdp, !!sym(country1)) %>%
    dplyr::pull()

  gdp <- dplyr::select(trade_gdp, !!sym(value2)) %>% dplyr::pull()

  trade <- dplyr::select(trade_gdp, -!!sym(country1), -!!sym(value2)) %>%
    as.matrix()
  trade[is.na(trade)] <- 0
